    ...
    async def timein(self,ctx, *, city):
        ...
        if timezone == False:
            ...
            try:
                ...
                req = requests.get(url=url, timeout=5.0)
                resp_time = req.json()
                ...
                date = datetime.datetime.strptime(resp_time['datetime'][:10], '%Y-%m-%d') 
                # datetime object to string, turned into object to change format of the date in string form 
                date = datetime.datetime.strftime(date,'%A %B %d, %Y')  
                ...
                await msg.edit(content = 'Timed out. Please ensure you have inputted a valid city.')


    @commands.command(pass_context=True)
    async def weather(self,ctx,city,code=""):
        if code != "":
            weather_url = "https://api.openweathermap.org/data/2.5/forecast?q={},{}&appid={}&units=metric".format(city,code,WEATHER_API_KEY)
        else:
